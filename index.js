...
// --- صفحة السيرفرات المحسنة لتجنب الأخطاء ---
app.get('/servers', (req, res) => {
    if (!req.isAuthenticated() || !req.user) return res.redirect('/login');
    
    // جلب السيرفرات التي يمتلكها البوت وتظهر للمستخدم بسلامة
    const guilds = req.user.guilds || [];
    
    let html = `
        <div style="font-family: Arial; padding: 30px; background: #1a1a1a; color: white; min-height: 100vh;">
            <h2>مرحباً، ${req.user.username || 'مستخدم'}! اختر سيرفر للبدء في إدارته:</h2>
